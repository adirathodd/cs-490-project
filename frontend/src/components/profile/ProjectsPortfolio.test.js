...
jest.mock('../../services/api', () => {
  ...
  return { projectsAPI: mockProjectsAPI };
});

jest.mock('../common/Icon', () => ({ name }) => (
...
describe('ProjectsPortfolio', () => {
  ...
  it('applies query params, fetches projects, and renders cards', async () => {
    ...
    mockProjectsAPI.getProjects.mockResolvedValueOnce(sampleProjects);

    renderPortfolio();

    await waitFor(() => expect(mockProjectsAPI.getProjects).toHaveBeenCalledTimes(1));
  ...
  it('shows loading indicator while fetching and replaces it with data', async () => {
    ...
    mockProjectsAPI.getProjects.mockReturnValueOnce(pendingFetch);
  ...
  it('shows error banner when fetching projects fails', async () => {
    mockProjectsAPI.getProjects.mockRejectedValueOnce(new Error('Network error'));
  ...
  it('toggles technology chips and refetches projects with updated filters', async () => {
    mockProjectsAPI.getProjects.mockResolvedValue(sampleProjects);
    ...
    const lastCall = mockProjectsAPI.getProjects.mock.calls.at(-1)[0];
    ...
    const finalCall = mockProjectsAPI.getProjects.mock.calls.at(-1)[0];
  ...
  it('renders placeholders when project metadata is missing', async () => {
    ...
    mockProjectsAPI.getProjects.mockResolvedValueOnce(sparseProjects);
  ...
  it('clears all filters when reset is clicked', async () => {
    ...
    const lastFetchParams = mockProjectsAPI.getProjects.mock.calls.at(-1)[0];
  ...
  it('falls back to empty list when getProjects function is missing', async () => {
    const original = mockProjectsAPI.getProjects;
    mockProjectsAPI.getProjects = null;

    renderPortfolio();

    expect(await screen.findByText(/No matching projects/i)).toBeInTheDocument();

    mockProjectsAPI.getProjects = original;
  });
});
